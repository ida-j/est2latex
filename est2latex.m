...
function f=est2latex(est,rownames,colnames,caption,filename)
...
fprintf(FID, '%s& %.3f%s ',rownames{(i-1)/2+1},est(i,1), stars{i,1});
     else % even row - s.e.
fprintf(FID, '& (%.3f) ',est(i,1));
     end
    
    
    for j=2:(m-1) % columns
        if mod(i,2) == 1 % odd row - estimated coefficients
fprintf(FID, '& %.3f%s ',est(i,j), stars{i,j});
        else % even row - s.e.
fprintf(FID, '& (%.3f) ',est(i,j));
        end
    end
